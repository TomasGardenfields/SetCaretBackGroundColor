========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : SetCaretBackGroundColor
========================================================================


AppWizard が生成した SetCaretBackGroundColor には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには SetCaretBackGroundColor を構成している各ファイルの概要説明が含まれています。

SetCaretBackGroundColor.dsp
    このファイル (プロジェクト ファイル) はプロジェクト レベルの情報を含み、
    シングル プロジェクトまたはサブ プロジェクトのビルドに使用されます。他の
    ユーザーとプロジェクト ファイル (.dsp) を共有できますが、メイク ファイル
    はローカルにエクスポートしてください。

SetCaretBackGroundColor.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また CSetCaretBackGroundColorApp アプリケーションクラスを宣
    言します。

SetCaretBackGroundColor.cpp
    このファイルは CSetCaretBackGroundColorApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。

SetCaretBackGroundColor.rc
...
res\SetCaretBackGroundColor.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル SetCaretBackGroundColor.rc によってインクルード
    されます。

res\SetCaretBackGroundColor.rc2
...
SetCaretBackGroundColorDlg.h, SetCaretBackGroundColorDlg.cpp - ダイアログ
    これらのファイルは CSetCaretBackGroundColorDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な SetCaretBackGroundColor.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) SetCaretBackGroundColor.pch 
